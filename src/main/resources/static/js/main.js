...
const skillIconMap = {
    ...
    'C':                    { type: 'devicon', cls: 'devicon-c-plain colored' },
    'SQL':                  { type: 'emoji',   val: '🗄️' },
    // Frameworks
    'Spring Boot':          { type: 'devicon', cls: 'devicon-spring-plain colored' },
    '.NET':                 { type: 'devicon', cls: 'devicon-dot-net-plain colored' },
    'ASP.NET':              { type: 'devicon', cls: 'devicon-dot-net-plain colored' },
    'Entity Framework':     { type: 'devicon', cls: 'devicon-dot-net-plain colored' },
    // Banco de Dados
    'MySQL':                { type: 'devicon', cls: 'devicon-mysql-plain colored' },
    'SQL Server':           { type: 'devicon', cls: 'devicon-microsoftsqlserver-plain colored' },
    // Ferramentas
    ...
    // Redes
    'TCP/IP':               { type: 'emoji',   val: '🌐' },
    'DHCP':                 { type: 'emoji',   val: '🌐' },
    'DNS':                  { type: 'emoji',   val: '🌐' },
    'IPv4':                 { type: 'emoji',   val: '🌐' },
    'Active Directory':     { type: 'emoji',   val: '🔐' },
    'VPN':                  { type: 'emoji',   val: '🔒' },
    'NSO':                  { type: 'emoji',   val: '🔧' },
    'Winbox':               { type: 'emoji',   val: '🔧' },
    // Virtualização
    'VirtualBox':           { type: 'devicon', cls: 'devicon-virtualbox-plain colored' },
    'Hyper-V':              { type: 'devicon', cls: 'devicon-windows11-original colored' },
    'Windows Server':       { type: 'devicon', cls: 'devicon-windows11-original colored' },
    // Metodologias
    'Scrum':                { type: 'emoji',   val: '🔄' },
    'Kanban':               { type: 'emoji',   val: '📋' },
    // Idiomas
    'Português (Nativo)':   { type: 'emoji',   val: '🇧🇷' },
    'Inglês Avançado (TOEIC B1)': { type: 'emoji', val: '🇺🇸' },
};

function skillIcon(name) {
    const icon = skillIconMap[name];
    if (!icon) return '';
    if (icon.type === 'devicon') return `<i class="${icon.cls}"></i>`;
    return `<span class="skill-emoji">${icon.val}</span>`;
}
    ...
    grid.innerHTML = skills.map((s, i) => `
        <div class="skill-card" style="animation-delay:${i * 0.1}s">
            ...
            <div class="skill-items">
                ${s.items.map(item => `<span class="skill-badge">${skillIcon(item)}${item}</span>`).join('')}
            </div>
        </div>
    `).join('');
}
